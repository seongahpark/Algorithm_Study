...
int changeCount[MAXSIZE][MAXSIZE] = {}; // 해당 지점까지 이동했을 때 검 -> 흰방으로 바꾼 최소 개수
...
void dijkstra(int N, pair<int,int> start)
{
    ...
    while (!pq.empty())
    {
        ...
        for (auto& dir : Directions)
        {
            ...
            {
                // 흰 방이라면 next 위치가 처음 들린 곳이거나, 방 교체 횟수가 더 크다면 변경 
            ...
            {
                // 검은 방이라면 해당 위치를 흰방으로 바꿨을 때(+1) next 위치가 처음 들린 곳이거나, 방 교체 횟수가 더 크다면 변경 
